...
# Date:                Wed, 17 Jun 2026   Pseudo R-squ.:                  0.4215
# Time:                        03:28:10   Log-Likelihood:                -2323.8
...
# 2 나이가 1 증가할 때 이직할 오즈비
import numpy as np

odds_ratio = np.exp(model.params["age"])
# print(round(odds_ratio, 3)) # 0.996

# 3 새로운 직원(age=40, income=4500, overtime=1)에 대해, 모델을 이용하여 이직 확률을 예측
new = pd.DataFrame({"age":[40], "income":[4500], "overtime":[1]})
pred = model.predict(new)
print(round(pred, 3)) # 0.697