...
# 1. 결측치가 가장 많은 컬럼을 찾으시오 -> S2_Temp

# 2. 해당 컬럼의 결측치를 중앙값으로 대체
med = df["S2_Temp"].dropna()
# print(med)
med_n = med.median()
# print(med_n)
df["S2_Temp"] = df["S2_Temp"].fillna(med_n)
print(df["S2_Temp"].shape)

# 3. 결측치를 대체한 후 해당 컬럼의 평균을 소수점 셋째 자리까지 구하시오. -> 25.055
